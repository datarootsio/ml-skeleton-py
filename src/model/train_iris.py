...
def train(model_filename, input_data_filename):
    ...
    model_location = os.path.join(s.MODEL_DIR, "{}.extjoblib".format(model_filename))
    feature_names = iris_X.columns.values.tolist()

    testing_strategy = (
        "5-fold cross validation, using mean "
    ...
    metadata = ModelMetadata(
        ...
        regr_model,
        model_description,
        get_data_location(input_data_filename),
        None,
        feature_names,
        list(zip(feature_names, regr_model.feature_importances_)),
        testing_strategy,
        None,
    ...
    extjoblib.dump(regr_model, model_location)
